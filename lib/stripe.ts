...
const priceId = "price_1PU239ChYKNOKSxWjU5maTji";
...
export const createCheckoutSession = async ({ email }: { email: string }) => {
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "subscription",
      success_url: `https://product-hunt-clone-lake.vercel.app/new-product`,
      cancel_url: `https://product-hunt-clone-lake.vercel.app`,
    });
  ...
  try {
   ...
   if (!customer || !customer.id) {
    ...
    const portal = await stripe.billingPortal.sessions.create({
      ...
      return_url: `https://product-hunt-clone-lake.vercel.app`,
    });
