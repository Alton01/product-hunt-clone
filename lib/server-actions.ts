...
export const createProduct = async ({name, slug, headline, description, logo, 
    releaseDate, website, twitter, discord, images, category}: ProductData): Promise<any> => {
        try {
            const authenticatedUser = await auth()

            if (!authenticatedUser) {
                throw new Error("User must be signed in to create a product")
            }
